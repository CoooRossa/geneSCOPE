...
#' @description
#' Process a \code{plotDendroNetwork} output object to extract        if (verbose) message("[geneSCOPE::getDendroWalkPaths] Step 4b - Selected ", length(sel), " paths containing both ", gene, " and ", gene2, " from random walks")
    } else {
        if (verbose) message("[geneSCOPE::getDendroWalkPaths] Step 4b - No random walks found; enumerating and filtering paths containing both genes (cutoff=", cutoff, ", max_paths=", max_paths, ")")
        sel <- enumerate_paths_filter(function(p) gene %in% p && gene2 %in% p)
        if (verbose) message("[geneSCOPE::getDendroWalkPaths] Step 4b - Enumerated and filtered ", length(sel), " paths")rmative paths:
#' \itemize{
...
getDendroWalkPaths <- function(dnet_obj,
                ...
                if (length(ps)) {
                    for (pp in ps) {
                        ...
                        if (filter_fun(path_chr)) {
                            ...
                            kept[[k]] <- path_chr
                            if (k >= max_paths) {
                                if (verbose) message("[geneSCOPE::getDendroWalkPaths] Step 3 - Reached max_paths=", max_paths, "; stopping enumeration")
                                return(kept)
                            }
                        }
                    }
    ...
    if (!pair_mode) {
        if (has_walks) {
            ...
            if (verbose) message("[geneSCOPE::getDendroWalkPaths] Step 4a - Selected ", length(sel), " paths containing ", gene, " from random walks")
        } else {
            if (verbose) message("[geneSCOPE::getDendroWalkPaths] Step 4a - No random walks found; enumerating graph paths (cutoff=", cutoff, ", max_paths=", max_paths, ")")
            sel <- enumerate_paths_filter(function(p) gene %in% p)
            if (verbose) message("[geneSCOPE::getDendroWalkPaths] Step 4a - Enumerated and filtered ", length(sel), " paths")
        }
        # de-duplicate
        if (length(sel)) {
    ...
    if (has_walks) {
        ...
        if (verbose) message("[geneSCOPE::getDendroWalkPaths] Step 4b - Selected ", length(sel), " paths containing both ", gene, " and ", gene2, " from random walks")
    } else {
        if (verbose) message("[geneSCOPE::getDendroWalkPaths] Step 4b - No random walks found; enumerating and filtering paths containing both genes (cutoff=", cutoff, ", max_paths=", max_paths, ")")
        sel <- enumerate_paths_filter(function(p) (gene %in% p) && (gene2 %in% p))
        if (verbose) message("[geneSCOPE::getDendroWalkPaths] Step 4b - Enumerated and filtered ", length(sel), " paths")
    }
    # de-duplicate
    if (length(sel)) {
        key <- vapply(sel, paste, collapse = "->", FUN.VALUE = character(1))
