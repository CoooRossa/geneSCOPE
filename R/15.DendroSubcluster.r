...
#' @examples
#' \dontrun{
#' p <- plotDendroNetworkWithBranches(scope_obj,
#'     grid_name = "grid30",
...
plotDendroNetworkWithBranches <- function(
    ...
    if (verbose) message("[geneSCOPE::plotRWDendrogram] Constructing basic tree network")
    base_args <- list(
        scope_obj = scope_obj,
        grid_name = grid_name,
    ...
    if (verbose) message("[geneSCOPE::plotRWDendrogram] Preparing cluster labels")
    Vnames <- vnames
    ...
    if (!is.null(cluster_vec)) {
        ...
        if (verbose) message("[geneSCOPE::plotRWDendrogram] No cluster_vec provided, using single cluster C1")
    }
    names(clu) <- Vnames
    ...
    if (!enable_subbranch) {
        if (verbose) message("[geneSCOPE::plotRWDendrogram] enable_subbranch=FALSE, returning base network")
        return(list(
        ...
        tryCatch(
            {
                ...
                if (verbose) {
                    message(
                        "[geneSCOPE::plotRWDendrogram] Cluster ", cid, " subgraph: nodes=",
                ...
                if (vcount_sg >= 3) {
                    tryCatch(
                        ...
                        error = function(e) {
                            if (verbose) {
                                message("[geneSCOPE::plotRWDendrogram] Articulation method failed for cluster ", cid, ": ", e$message)
                            }
                        }
                    )
                }
    ...
    if (!length(target_clusters)) {
        if (verbose) message("[geneSCOPE::plotRWDendrogram] No available target clusters, skipping subdivision")
    ...
    if (enable_subbranch) {
        if (verbose) message("[geneSCOPE::plotRWDendrogram] Processing ", length(target_clusters), " clusters")
        for (cid in target_clusters) {
            ...
            if (length(genes_target_raw) != length(genes_target) && verbose) {
                message(
                    "[geneSCOPE::plotRWDendrogram] Cluster ", cid, ": Filtered genes ",
                    length(genes_target_raw), " -> ", length(genes_target)
                )
            }

            if (length(genes_target) < 3) {
                if (verbose) message("[geneSCOPE::plotRWDendrogram] Cluster ", cid, ": Too few nodes, skipping")
                next
            }
            ...
            if (verbose) {
                message(
                    "[geneSCOPE::plotRWDendrogram] Cluster ", cid, ": method=", res_c$method,
    ...
    if (verbose) message("[geneSCOPE::plotRWDendrogram] Generating final plot")
    branch_network <- NULL
