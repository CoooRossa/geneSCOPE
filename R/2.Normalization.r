...
#' @return The modified \code{scope_object} containing a new \code{$Xz} slot.
#' @details
...
#' @title Add log-CPM normalized matrix to a scope_object
#' @description
...
normalizeSingleCells <- function(scope_obj,
                                 ...
                                 scale_factor = 1e4,
                                 verbose = TRUE) {
  if (verbose) message("[geneSCOPE::normalizeSingleCells] Starting log-CPM normalization...")
  ...
  if (verbose) message("[geneSCOPE::normalizeSingleCells] Computing library sizes...")
  lib <- Matrix::colSums(mat) # Library size per column
  zerocell <- lib == 0
  lib[zerocell] <- 1 # Avoid division by zero

  if (verbose) message("[geneSCOPE::normalizeSingleCells] Applying CPM transformation and log1p...")
  ...
  if (verbose) message("[geneSCOPE::normalizeSingleCells] Log-CPM normalization completed, stored in @cells$", output_layer)
