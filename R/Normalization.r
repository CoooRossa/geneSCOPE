...
#' @title Add log-CPM normalised matrix to a CoordObj
#'
#' @description
...
#' @export
normalizeCells_CPMlog <- function(coordObj,
                                  input_layer  = "counts",
                                  output_layer = "logCPM",
                                  scale_factor = 1e4)
{
  stopifnot(inherits(coordObj, "CoordObj"),
  ...
  if (any(lib == 0))
    warning("Some cells have zero total counts; they remain zero after normalisation.")

  out <- mat
  out@x <- out@x / rep.int(lib, diff(out@p)) * scale_factor
  out@x <- log1p(out@x)                        # log1p CPM
